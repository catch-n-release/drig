...
from drig.utils import display_image, image_cast, list_image_paths, image_class, random_image
import cv2
import numpy as np
...
def test_image_cast(random_testing_image_path):
    assert image_cast(random_testing_image_path) == cv2.imread(
...
def test_image_class(random_testing_image_path):
    true_class_name = random_testing_image_path.split("/")[-2]
    class_name = image_class(random_testing_image_path, -3)
    assert true_class_name == class_name


def test_random_image_from_dataset():

    image, image_path, class_name = random_image(
...
def test_random_image_from_image_paths():
    all_image_paths = list_image_paths(config.DATASET_PATH)
    image, image_path, class_name = random_image(
